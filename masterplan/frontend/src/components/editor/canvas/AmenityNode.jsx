...
import { Group, Line, Ellipse, Rect, Circle, Shape, Image, Text } from 'react-konva';

export default function AmenityNode({
  amenity,
  isSelected,
  isClusterSelected,
  activeTool,
  ...
  handleAmenityTransformEnd,
  getLegendNumber,
  getAccessTextureKey,
  getAmenityTextureKey,
  ...
  const rad = ((amenity.rotation_deg || 0) * Math.PI) / 180;
  ...
  const unrotXs = localPts.map(p => p[0]);
  const unrotYs = localPts.map(p => p[1]);
  ...
  const isSpecialPoint = amenity.type === 'tree' || amenity.type === 'entry_exit' || amenity.type === 'boundary_trees';
    ...
    >
      ...
      {isLawnOrPark ? (
        amenity.shape === 'ellipse' ? (
          <Group x={bbox.cx} y={bbox.cy} rotation={amenity.rotation_deg || 0}>
            ...
            <Group listening={false} x={bbox.cx} y={bbox.cy} rotation={angleDeg}>
              {amenity.type === 'tree' ? (
                <Image
                  x={0}
                  y={0}
                  width={Math.max(tw, th) * 1.2}
                  height={Math.max(tw, th) * 1.2}
                  offsetX={Math.max(tw, th) * 0.6}
                  offsetY={Math.max(tw, th) * 0.6}
                  image={treeTemplates.medium}
                  listening={false}
                />
              ) : (
                <>
